...
static NES_VOLUME_HANDLER s_n106_volume_handler[] = {
	{ N106SoundVolume, NES_SOUND_TAG,},
	{ 0, 0, },
};


__inline static void UPDATE(N106_WM *chp,int i)
{
	if (chp->update & 3)
	{
		Uint32 freq;
        ...
        Uint32 tlen;
        if (nes_devif.n163_oldmode)
            tlen = (0x20 - (chp->freqh & 0x1c));
		else
            tlen = (0x100 - (chp->freqh & 0xfc));
        
        chstate[i].freq = 0;
        float clk = (15 * (65536/4) * tlen * n106s.chinuse);

        if (clk > 0)
        {
            float result = ((float)freq / clk) * (NES_CPUCLOCK/4);
            chstate[i].freq = result;
        }
	}
	if (chp->update & 2)
	{
...
static Int32 __fastcall N106SoundRender(void)
{
	...
	return VOLGAIN(accum,s_n106_volume_handler[0]);
}
