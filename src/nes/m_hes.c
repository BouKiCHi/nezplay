...
/*	if (fp) { 
	...
	if (fp) {
		fprintf(fp,"PC:%04x A:%02x X:%02x Y:%02x\n",pc,a,x,y);
		p++;
	}
}
...
static Uint32 Callback read_event(void *sp, Uword a)
{
	HESHES *THIS_ = sp;
...
static void Callback write_event(void *sp, Uword a, Uword v)
{
	HESHES *THIS_ = sp;
...
static Uint32 Callback readmpr_event(void *sp, Uword a)
{
	HESHES *THIS_ = sp;
...
static void Callback writempr_event(void *sp, Uword a, Uword v)
{
	HESHES *THIS_ = sp;
...
static void Callback write6270_event(void *sp, Uword a, Uword v)
{
	HESHES *THIS_ = sp;
