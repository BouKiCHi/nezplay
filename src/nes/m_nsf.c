...
/* RAM area */
static Uint  ReadRam(Uint A)
{
	return nsf.ram[A & 0x07FF];
}
static void WriteRam(Uint A, Uint V)
{
	nsf.ram[A & 0x07FF] = V;
}

/* SRAM area */
static Uint ReadStaticArea(Uint A)
{
	return nsf.static_area[A - 0x6000];
}
static void WriteStaticArea(Uint A, Uint V)
{
	nsf.static_area[A - 0x6000] = V;
}

#if !NSF_MAPPER_STATIC
static Uint __fastcall ReadRom8000(Uint A)
{
	return nsf.bank[0][A];
}
static Uint __fastcall ReadRom9000(Uint A)
{
	return nsf.bank[1][A];
}
static Uint __fastcall ReadRomA000(Uint A)
{
	return nsf.bank[2][A];
}
static Uint __fastcall ReadRomB000(Uint A)
{
	return nsf.bank[3][A];
}
static Uint __fastcall ReadRomC000(Uint A)
{
	return nsf.bank[4][A];
}
static Uint __fastcall ReadRomD000(Uint A)
{
	return nsf.bank[5][A];
}
static Uint __fastcall ReadRomE000(Uint A)
{
	return nsf.bank[6][A];
}
static Uint __fastcall ReadRomF000(Uint A)
{
	return nsf.bank[7][A];
}
#endif
...
/* Mapper I/O */
static void WriteMapper(Uint A, Uint V)
{
	Uint32 bank;
	bank = A - 0x5FF0;
...
Uint NSFReadLinear(Uint A)
{
    if ((A>>12) < nsf.banknum)
    {
        return nsf.bankbase[A];
    }
    return 0;
}
...
static Uint NSFDeviceInitialize(void)
{
    ...
    MASKSET(nsf_state.apu_mask);
    MASKSET(nsf_state.vrc6_mask);
    MASKSET(nsf_state.vrc7_mask);
    MASKSET(nsf_state.fds_mask);
    MASKSET(nsf_state.mmc5_mask);
    MASKSET(nsf_state.n106_mask);
    MASKSET(nsf_state.fme7_mask);
    
    ZEROSET(nsf_state.apu_state);
    ZEROSET(nsf_state.vrc6_state);
    ZEROSET(nsf_state.vrc7_state);
    ZEROSET(nsf_state.fds_state);
    ZEROSET(nsf_state.mmc5_state);
    ZEROSET(nsf_state.n106_state);
    ZEROSET(nsf_state.fme7_state);
    
 	APUSoundInstall();
    APUSetMask(nsf_state.apu_mask);
    APUSetState(nsf_state.apu_state);

	if (SONGINFO_GetExtendDevice() & EXTSOUND_VRC6)
    {
        VRC6SoundInstall();
        VRC6SetMask(nsf_state.vrc6_mask);
        VRC6SetState(nsf_state.vrc6_state);
    }
    if (SONGINFO_GetExtendDevice() & EXTSOUND_VRC7)
    {
        VRC7SoundInstall();
        VRC7SetMask(nsf_state.vrc7_mask);
        VRC7SetState(nsf_state.vrc7_state);
    }
    if (SONGINFO_GetExtendDevice() & EXTSOUND_FDS)
    {
        FDSSoundInstall();
        FDSSetMask(nsf_state.fds_mask);
        FDSSetState(nsf_state.fds_state);
    ...
    if (SONGINFO_GetExtendDevice() & EXTSOUND_MMC5)
	{
        ...
        MMC5SetMask(nsf_state.mmc5_mask);
        MMC5SetState(nsf_state.mmc5_state);

	}
	if (SONGINFO_GetExtendDevice() & EXTSOUND_N106)
    {
        N106SoundInstall();
        N106SetMask(nsf_state.n106_mask);
        N106SetState(nsf_state.n106_state);
        
    }
	if (SONGINFO_GetExtendDevice() & EXTSOUND_FME7)
    {
        FME7SoundInstall();
	    FME7SetMask(nsf_state.fme7_mask);
        FME7SetState(nsf_state.fme7_state);
...
static Uint NSFPlayerSetInfo(Uint8 *pData, Uint uSize)
{
    ...
    SONGINFO_SetNSFflag(SONGINFO_NSF);
    SONGINFO_SetTitle((char *)pData + 0xe);
...
static void NSFSetReadLinearHandler()
{
