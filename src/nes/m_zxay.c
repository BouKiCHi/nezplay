...
static Uint32 ioread_event(void *sp, Uint32 a)
{
	return 0xff;
}
...
static void iowrite_event(void *sp, Uint32 a, Uint32 v)
{
	ZXAY *THIS_ = sp;
...
static void reset(ZXAY *THIS_)
{
	...
	do
	{
		...
		if (p2 + size > THIS_->datalimit)
			size = (Uint32)(THIS_->datalimit - p2);
	...
	THIS_->ctx.regs8[REGID_A] = THIS_->ctx.regs8[REGID_B] = THIS_->ctx.regs8[REGID_D] = THIS_->ctx.regs8[REGID_H] = THIS_->ctx.regs8[REGID_IXH] = THIS_->ctx.regs8[REGID_IYH] = (reginit >> 8) & 0xff;
	THIS_->ctx.regs8[REGID_F] = THIS_->ctx.regs8[REGID_C] = THIS_->ctx.regs8[REGID_E] = THIS_->ctx.regs8[REGID_L] = THIS_->ctx.regs8[REGID_IXL] = THIS_->ctx.regs8[REGID_IYL] = (reginit >> 0) & 0xff;
...
static Int32 __fastcall ZXAYSoundRenderMono(void)
{
	Int32 d[2] = { 0, 0 };
	synth(zxay, d);
#if (((-1) >> 1) == -1)
...
static NES_AUDIO_HANDLER zxay_audio_handler[] = {
...
static NES_VOLUME_HANDLER zxay_volume_handler[] = {
	{ ZXAYVolume, "ZXAY" },
	{ 0,0 },
};
...
static NES_RESET_HANDLER zxay_reset_handler[] = {
	{ NES_RESET_SYS_LAST, ZXAYReset, },
	{ 0,                  0, },
};

static void __fastcall ZXAYTerminate(void)
{
	if (zxay)
	{
		terminate(zxay);
...
static NES_TERMINATE_HANDLER zxay_terminate_handler[] = {
	{ ZXAYTerminate, },
	{ 0, },
};
...
Uint32 ZXAYLoad(Uint8 *pData, Uint32 uSize)
{
	Uint32 ret;
	ZXAY *THIS_;
	// if (zxay) *((char *)(0)) = 0;	/* ASSERT */
	THIS_ = XMALLOC(sizeof(ZXAY));
